...
<body class="flex items-center justify-center min-h-screen font-sans bg-gray-300">
    ...
    <div id="contentDiv" class="w-3/4 p-10 mt-4 bg-white shadow-lg rounded-xl">
        <h1 class="mb-6 text-2xl font-bold text-green-700 text-start">Table 2</h1>
        <form method="POST" action="{{ route('store.service.type.2') }}">
            @csrf
        ...
        {{-- Table 2 for Queue and Recall Number --}}
        <table class="w-full text-center bg-gray-300 table-fixed">
            {{-- Get Next Number for table 2 (Queue 2) --}}
            <tbody>
                <tr>
                    <td class="flex items-center justify-center px-6 py-4 border-b border-gray-500">
                        <div
                            ...
                            {{ session('current_number_2', 1) }}
                        </div>
                    </td>
                    <td class="px-6 py-4 border-b border-gray-500">
                        <form method="POST" action="{{ route('get-number-2') }}" class="inline-block">
                            @csrf
                            <button type="submit" class="btn btn-base btn-success">Place Number</button>
                        </form>
                        <form method="POST" action="{{ route('reset-number-2') }}" class="inline-block ml-2">
                            @csrf
            ...
            {{-- end get next number for table 2 --}}

            {{-- Recall Number for Table 2 --}}
            <tbody>
                <tr>
                    <td class="p-12 px-6">
                        <form method="POST" action="{{ route('recall-number-2') }}" class="inline-block">
                            @csrf
                            <div class="text-xl font-bold">
                                <input type="number" min="1" max="25" name="recalled_number_2"
                    ...
                    <td class="px-6">
                        <form method="POST" action="{{ route('reset-recall-number-2') }}" class="inline-block ml-2">
                            @csrf
                            <button type="submit" class="btn btn-base btn-error">Reset Number</button>
                        </form>
                    </td>
