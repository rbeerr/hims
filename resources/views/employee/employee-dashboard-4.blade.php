...
<body class="flex items-center justify-center min-h-screen font-sans bg-gray-300">
    ...
    <nav id="navbar" class="absolute top-0 flex items-center justify-between w-full px-4 py-2 bg-green-800">
        ...
        <div class="mr-20 dropdown dropdown-hover">
            ...
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-15">
                ...
                </li><button onclick="initializeClientDashboard()"
                class="block w-full px-4 py-2 text-left text-black hover:bg-green-200 focus:outline-none">
                Client Dashboard&#9656;
            </button>
                <li>
    ...
    <div id="contentDiv" class="w-3/4 p-10 mt-4 bg-white shadow-lg rounded-xl">
        <h1 class="mb-6 text-2xl font-bold text-green-700 text-start">Table 4</h1>

        <form method="POST" action="{{ route('store.service.type.4') }}">
            @csrf
                    ...
                    <tr>
                        <td class="px-6 py-4">
                            <select name="service_type"
                                ...
                                <option>WCPU</option>
                                <option>ECG/EEG/EMG Result</option>
        ...
        {{-- Table 4 for Queue and Recall Number --}}
        <table class="w-full text-center bg-gray-300 table-fixed">
            {{-- Get Next Number for table 4 --}}
            <tbody>
                <tr>
                    <td class="flex items-center justify-center px-6 py-4 border-b border-gray-500">
                        <div
                            ...
                            {{ session('current_number_4', 1) }}
                        </div>
                    </td>
                    <td class="px-6 py-4 border-b border-gray-500">
                        <form method="POST" action="{{ route('get-number-4') }}" class="inline-block">
                            @csrf
                            <button type="submit" class="btn btn-base btn-success">Place Number</button>
                        </form>
                        <form method="POST" action="{{ route('reset-number-4') }}" class="inline-block ml-2">
                            @csrf
            ...
            {{-- end get next number for table 4 --}}

            {{-- Recall Number for Table 4 --}}
            <tbody>
                <tr>
                    <td class="p-12 px-6">
                        <form method="POST" action="{{ route('recall-number-4') }}" class="inline-block">
                            @csrf
                            <div class="text-xl font-bold">
                                <input type="number" min="1" max="100" name="recalled_number_4"
                    ...
                    <td class="px-6">
                        <form method="POST" action="{{ route('reset-recall-number-4') }}" class="inline-block ml-2">
                            @csrf
                            <button type="submit" class="btn btn-base btn-error">Reset Number</button>
                        </form>
                    </td>
